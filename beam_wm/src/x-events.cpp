 ...
 *
 * Case edit remove the requirement of the mouse to MAXIMIZE
 * Currently, it chooses the client window (line 48) based upon
 * what window X says the key event occurred in (which is dictated
 * by the position of the mouse, as you know), but you could easily
 * change which client is used.
 * Fix:
 * replace the subwindow from the xkeyEvent with the current Focus
 * get_input_focus() in xdata.cpp ret curr_foc window
 * OLD Client get version: Window client = m_event.xkey.subwindow;
 */
 /*
...
//    Window client = m_event.xkey.subwindow;
    Window client = m_xdata.get_input_focus(); // Use only the Current Focused Client window
    if (client == None)
        client = m_event.xkey.window;          // Probable need to replace or remove this
    ...
    case K_SNAP_RIGHT:
        ...
        break;
    case K_SNAP_RIGHT_BOTTOM:
        if (is_client)
            snap_client(client, SNAP_RIGHT_BOTTOM);
        break;
    case LAYER_ABOVE:
        if (is_client)
...
void XEvents::maximize_client(Window window)
{
    ...
    m_clients.change_location(window, 0, 0);
    //m_clients.change_size(window, scr_width, scr_height - m_config.icon_height);
    m_clients.change_size(window, scr_width, scr_height);
}
...
void XEvents::snap_client(Window window, SnapDir side)
{
    ...
    /* Dimension workspace_height = scr_height - m_config.icon_height;
     * Need to use the Full Screen in this Project so remove the - m_config.icon_height and use scr_height
     * Old
     *    case SNAP_TOP:
     *       m_clients.change_location(window, 0, m_config.icon_height);
    ...
    case SNAP_TOP:
        m_clients.change_location(window, 0, 0);
        m_clients.change_size(window, scr_width, scr_height / 2);
        break;
    case SNAP_BOTTOM:
    ...
    case SNAP_LEFT:
        m_clients.change_location(window, 0, 0);
        m_clients.change_size(window, scr_width / 2, scr_height);
        break;
    case SNAP_RIGHT:
        m_clients.change_location(window, scr_width / 2, 0);
        m_clients.change_size(window, scr_width / 2, scr_height);
        break;
    case SNAP_RIGHT_BOTTOM:
        m_clients.change_location(window, scr_width / 2, scr_height / 2);
        m_clients.change_size(window, scr_width / 2, scr_height / 2);
        break;
    }
}
